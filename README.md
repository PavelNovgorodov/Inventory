...
### 1.2. Цели создания
- **Автоматизация процессов инвентаризации:** Устранение ручного труда при сборе данных об аппаратном и программном обеспечении, настройках ОС.
- **Обеспечение аудита и контроля:** Создание единого, актуального источника данных для аудита лицензий ПО, проверки соответствия политикам безопасности, планирования модернизации.
...
- **Создание основы для анализа:** Формирование структурированного хранилища данных для последующего анализа средствами BI (Excel Power Query).
...
| Термин | Определение |
|--------|-------------|
| АИ | Автоматизированный инструмент «Сбор инвентаризационной информации» |
| ИИ | Инвентаризационная информация - данные об аппаратном обеспечении, установленном ПО, настройках ОС и системе, собираемые с компьютера |
...
| Сырые данные | CSV-файлы с данными, собранными с одного конкретного компьютера. Формат: `<ИмяКомпьютера>.<ТипИИ>.csv` |
| Консолидированные данные | Итоговые CSV-файлы, объединяющие данные одного типа со всех компьютеров. Формат: `<ТипИИ>.csv` |
...
1.  **Гибридная модель сбора:** Система должна поддерживать **локальный** (запускаемый непосредственно на объекте сбора) и **удаленный** (запускаемый на компьютере-сборщике) сбор данных.
2.  **Консолидация в едином хранилище:** Все данные, независимо от метода и источника сбора (домен, изолированный сегмент, автономный компьютер), должны накапливаться и обрабатываться в единой структуре каталогов. Все рабочие каталоги системы могут указывать на сетевые ресурсы (UNC-пути).
...
4.  **Модульность:** Функциональность сбора каждого типа данных должна быть инкапсулирована в независимый, подключаемый модуль. Добавление нового типа данных не должно требовать изменения ядра системы.
5.  **Идемпотентность:** Повторный запуск сбора с одних и тех же источников не должен приводить к дублированию данных в конечных результатах.
...
2. **Гибкость**: Поддержка различных сценариев подключения и переноса данных
3. **Масштабируемость**: Возможность постепенного добавления новых источников данных
4. **Простота развертывания**: Не требует установки серверных компонентов или настройки баз данных
5. **Совместимость**: Единый формат данных для всех методов сбора
...
| **FR-05** | Консолидация и обработка данных | Объединение файлов от отдельных компьютеров в итоговые файлы по типам информации, удаление устаревших данных | `-StartType CreateResult`, `-DayOld` |
| <nobr>**FR-05.1** | Обработка ZIP архивов локального сбора | Автоматическое распаковка ZIP-архивов, помещенных в каталог `InvFolderZIPFiles`, и включение данных из них в процесс консолидации | (часть `CreateResult`) |
...
| <nobr>**NFR-08.1** | **Надежность** | Ротация лог файлов процедур удаленного сбора и консолидации данных | Логи ведутся в отдельных файлах по дате выполнения. Старые файлы удаляюся, система обеспечивает хранение не более 20 лог файлов |
...
| <nobr>**NFR-09** | **Безопасность** | Контроль доступа к данным | Права NTFS на каталогах с данными должны следовать принципу наименьших привилегий. |
...
test_scenario: |
  ...
  5. Открыть файл inv.xlsm, выполнить обновление данных, убедиться в наличии информации
```
...
context: |
  Для решения конкретных задач не требуется полный сбор всех 19 типов данных.
...
test_scenario: |
    ...
    4. Убедиться, что другие типы данных не собраны
    5. Выполнить команду:
...
benefit: "оптимизировать хранение данных"
context: |
...
test_scenario: |
    ...
    3. Открыть Excel-отчет, выполнить обновление данных
...
**US-05**: Как системный администратор, я хочу использовать разные методы сбора для разных сегментов сети (удаленный сбор для доверенного сегмента, локальный сбор для DMZ) из соображений безопасности.
   ...
   **Критерии приемки:**
     ...
     - Единый формат данных для обоих методов
     - Автоматическое объединение данных при обработке
...
test_scenario: |
    ...
    4. Запустить CreateResult для объединения всех данных
...
*Примечание: Альтернативные потоки, связанные с общесистемными механизмами (проверка доступности, прав, актуальности данных, создание каталогов), описаны в соответствующих включаемых сценариях (UC-INC-01 — UC-INC-04) и не дублируются в каждом основном Use Case. В основных сценариях приводятся только уникальные альтернативы, специфичные для данного потока.*
...
- **Актор:** Система
- **Описание:** Базовые проверки перед началом любой операции сбора или обработки данных
...
**UC-INC-02: Проверка актуальности данных**

- **Актор:** Система
- **Описание:** Проверка, требуется ли выполнять сбор для конкретного компьютера на основе возраста существующих данных
...
- **Основной поток (для каждого компьютера в списке):**
    ...
    4. Если все проверки пройдены, устанавливается CIM-сессия для сбора данных
...
- **Актор:** Admin
- **Описание:** Полный цикл удаленного сбора и консолидации данных со всех компьютеров в заданном OU домена
...
- **Предусловие:** Наличие прав администратора на компьютерах в домене, доступ к сетевому каталогу для хранения данных
- **Основной поток:**
    ...
    6. Для каждого компьютера из полученного списка:
        - Если указан параметр `InvRemoteDayOld`, система выполняет UC-INC-02 (Проверка актуальности данных)
        - Если требуется сбор (данные устарели или отсутствуют), система выполняет UC-INC-04 (Проверка доступности компьютера)
        - Если компьютер доступен:
            - Для каждого типа информации из полученного списка типов:
                ...
                * Выполняется удаленный сбор данных через установленную CIM-сессию
...
- **Альтернативные потоки:**
    1. Если после завершения сбора не было собрано данных ни с одного компьютера (все недоступны или все данные актуальны), система выводит предупреждение "Нет новых данных для консолидации" и завершает выполнение без создания итоговых файлов
- **Исключения:**
    ...
    2. Если при выполнении сбора произошла критическая ошибка, делающая невозможным продолжение, процесс прерывается, частично собранные данные сохраняются
...
- **Описание:** Сбор данных непосредственно на компьютере с последующей упаковкой в ZIP-архив для ручного переноса
...
- **Основной поток:**
    ...
    3. Система выполняет UC-INC-01 (Проверка и подготовка инфраструктуры)
    4. Система выполняет UC-INC-05 (Валидация типов собираемой информации), получая список типов информации
    5. Если указан параметр `DayOld`, система выполняет UC-INC-02 (Проверка актуальности данных) для данного компьютера
    6. Если требуется сбор (данные устарели или отсутствуют):
        - Для каждого типа информации из полученного списка типов:
            * Загружается соответствующий модуль из каталога `Get-Inventory-Module\`
            * Выполняется локальный сбор данных
            * Результат сохраняется в файл формата `ИмяКомпьютера.КодТипа.csv` в каталоге `InvFolderAnyComputers`
        - При ошибке в одном модуле сбор продолжается для остальных, ошибка логируется
...
- **Альтернативные потоки:**
    1. Если параметр `InvFolderZIPFiles` не указан, упаковка в архив не выполняется, данные остаются только в `InvFolderAnyComputers`
...
- **Актор:** Admin
- **Описание:** Сбор только указанных типов данных с конкретных компьютеров для решения узких задач
...
- **Основной поток:**
    ...
    7. Для каждого компьютера из полученного списка:
        - Если указан параметр `InvRemoteDayOld`, система выполняет UC-INC-02 (Проверка актуальности данных) для данного компьютера
...
- **Актор:** Manager
- **Описание:** Обработка ранее собранных данных, включая распаковку ZIP-архивов и консолидацию
- **Включает:** UC-INC-01
...
- **Основной поток:**
    ...
    4. Запускается процедура консолидации данных:
        - Если указан параметр `DayOld`, в каталоге `InvFolderAnyComputers` удаляются файлы инвентаризации старше указанного количества дней
        - Система сканирует каталог `InvFolderAnyComputers` и группирует файлы по типу информации
        - Для каждого типа информации создается итоговый файл `Тип.csv` в каталоге `InvFolderResult` 
- **Постусловие:** Консолидированные данные готовы для импорта в Excel и анализа
- **Альтернативные потоки:**
...
- **Актор:** Admin
- **Описание:** Комбинирование данных из различных источников (разные домены, DMZ, изолированные сети) в единую инвентаризационную базу
...
- **Основной поток:**
    1. **Сбор из основного домена:**
        ...
        - Для каждого компьютера из OU domain1 выполняется сбор данных (аналогично UC-01, шаг 6, без финальной консолидации)
    ...
    4. **Финальная консолидация:**
        - Администратор запускает UC-04 (Объединение данных) для обработки всех источников
...
| Основной сценарий (UC) | **UC-INC-01** (Проверка инфраструктуры) | **UC-INC-02** (Проверка актуальности) | **UC-INC-03** (Получение перечня компьютеров) | **UC-INC-04** (Проверка доступности) | **UC-INC-05** (Валидация типов собираемой информации) |
| :--- | :---: | :---: | :---: | :---: |
...
| **UC-04: Объединение данных для анализа** | Да | Нет | Нет | Нет | Нет |
...
| **FR-05** | Консолидация и обработка данных | UC-01, UC-04, UC-05 |
...
| **NFR-02** | Проверка актуальности уже собранных данных | UC-01, UC-02, UC-03, UC-04, UC-06 |
...
| **NFR-08.1** | Ротация лог файлов процедур удаленного сбора и консолидации данных | UC-01, UC-03, UC-04, UC-05 |
| **NFR-08.2** | Ротация лог файлов локального сбора | UC-02, UC-06 |
| **NFR-09** | Контроль доступа к данным | UC-01, UC-02, UC-04, UC-05, UC-06 |
...
| **US-04:** Объединение для анализа | **UC-04:** Объединение данных | UC-INC-01, UC-INC-02 |
...
> **Примечание:** Каждый Use Case включает базовые проверки инфраструктуры (UC-INC-01) и, при необходимости, проверку актуальности данных (UC-INC-02) если указан параметр `DayOld`.

## 7.6. BPMN-диаграмма процессов

![BPMN-диаграмма процессов](Docs/bpmn.png "Рисунок 1 – Процесс удаленного сбора")

## 8. Спецификация данных и форматов
...
Система включает 19 модулей сбора. Каждый модуль представляет собой скрипт PowerShell (`.ps1`) в каталоге `Src\Get-Inventory-Module\` и отвечает за сбор одного типа данных.

| Код | Наименование типа | Файл модуля | Краткое описание (Источник данных) |
| :--- | :--- | :--- | :--- |
| **hwr** | Аппаратная конфигурация | `hwr.ps1` | Сводные данные о системе: модель, ОС, процессор, память, сеть. Источники: WMI (`Win32_ComputerSystem`, `Win32_Processor`), реестр. |
...
- **Уникальность:**
  ...
  - Уникальность возвращаемых данных обеспечивается на стороне модуля сбора
...
| <nobr>-InvFolderAnyComputers | [String] | ".\InvAnyComputers\" | Локальный или UNC-путь к каталогу | Каталог для хранения сырых данных по отдельным компьютерам (файлы `ИмяПК.ТТТ.csv`). |
...
| <nobr>-InvFolderZIPFiles | [String] |  | Локальный или UNC-путь к каталогу | Каталог для ZIP-архивов от автономных компьютеров. Проверка актуальности данных при локальном сборе.|
| <nobr>-DayOld | [Int32] | 30 | Целое число ≥ 0 | Максимальный возраст файлов (в днях):<br>при локальном сборе – для пропуска сбора, если данные не старше N дней;<br>при удаленном сборе - при получении списка компьютеров из OU (компьютеры активные в течении N дней);<br>при  консолидации – для удаления файлов в `InvFolderAnyComputers` старше N дней. |
| <nobr>-InvRemoteDayOld | [Int32] | 0 | Целое число ≥ 0 | Проверка актуальности данных при удаленном сборе. Если данные младше указанного значения, сбор пропускается. 0 = не проверять. |
...
**Назначение:** Удаленный сбор данных с указанных компьютеров.
**Требования:**
...
3. Сбор данных через WinRM/CIM
4. Сохранение результатов в `InvFolderAnyComputers`

#### 9.3.2. `StartType = "InvLocal"`
**Назначение:** Локальный сбор данных на компьютере, где запущен командлет.
**Требования:** Права доступа к WMI и реестру (обычно SYSTEM)

**Поведение:**
1. Сбор данных с локального компьютера
...
**Назначение:** Обработка и консолидация собранных данных.
...
3. Объединение CSV-файлов по типам данных

#### 9.3.4. `StartType = "InvRemoteCreateResult"` (значение по умолчанию)
**Назначение:** Комбинированный сценарий - сбор + обработка.
**Поведение:** Сначала выполняется сбор данных со всех целевых компьютеров (как в `InvRemote`), а после завершения цикла сбора автоматически запускается процедура консолидации (как в `CreateResult`).

### 9.4. Форматы входных данных
...
- **Требование NFR-04:** Права NTFS на каталогах с данными должны соответствовать матрице прав.
