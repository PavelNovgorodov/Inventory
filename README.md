...
4. Проверка актуальности информации (если указан `-InvRemoteDayOld`)
5. Сохранение результатов в `InvFolderAnyComputers`
...
4. Проверка актуальности информации (если указан `-DayOld`)
...
- **Module = "shr"** (Общие ресурсы)
  ...
  - Глубина сбора NTFS-разрешений для конкретных компьютеров
    - `VarName = "DepthAnyComputer"`
    - `VarValue = "@(("PatternName", X), ("ComputerName", X))"` - массив с перечнем компьютеров с уровнем вложенности
      - `ComputerName` - имя компьютера
      - `X` - значение вложенности
      - `PatternName` - использование маски в имени компьютера (возможные варианты для оператора -like)
      - Если компьютер перечислен в нескольких значениях (в том числе по маске) - будет использоваться последнее значение

    Передача параметра:
    ```powershell
    $moduleParams = @(
                        @{ Module = "shr"; VarName = "DepthChildFolder"; VarValue = X }, 
                        @{ Module = "shr"; VarName = "DepthAnyComputer"; VarValue = @(("PatternName", X), ("ComputerName", X)) } 
                    )
    ```

- **Module = "reg"** (Значение ключей реестра)
  - `VarName = "Reg"`
  - `VarValue = @(@{HKey, Key, ValueName})` - массив хэш таблиц
    ...
    - Если `ValueName` не указан, собираются все значения ключа, включая вложенные
...
#### Пример 2: Локальный сбор для автономного компьютера и проверкой наличия актуальных файлов инвентаризации
```powershell
...
#### Пример 3: Выборочный сбор для аудита ПО с получением списка компьютеров из файла
```powershell
...
- Для удаленного сбора инвентаризационной информации требуется учетная запись с административными правами на целевых компьютерах
...
### 12.1. Платформенные ограничения
...
### 12.2. Архитектурные ограничения
...
### 12.3. Функциональные ограничения
