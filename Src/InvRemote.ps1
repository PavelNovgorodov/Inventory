...
$m3 = @(
            ...
            @{Module = "shr"; VarName = "DepthAnyComputer"; VarValue = ("fileservers*", 2), ("fileserver1", 2), ("fileserver2", 3), ("fileserver4", 0)}
        )
#Start-Inventory -StartType InvRemote -InvFolderAnyComputers"C:\Inventory\Src\InvAnyComputers" -ModuleParameters $m3

<#
    Запустить сбор значений параметров реестра с компьютеров перечисленных в файле computers.csv:
        HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Run - всех параметров включая вложенные ветки
        HKEY_USERS\SOFTWARE\Microsoft\Windows\CurrentVersion\Internet Settings, AutoConfigURL - значение параметра AutoConfigURL для всех профилей пользователей
#>
    $Parameters = @(
                        @{
                            Module = "reg";
                            ...
                            VarValue = @(
                                            ...
                                            @{HKey = "HKEY_USERS"; Key = "SOFTWARE\Microsoft\Windows\CurrentVersion\Internet Settings"; ValueName = "AutoConfigURL"}
                                         )
                         }
                    )
#Start-Inventory -StartType InvRemote -InvComputerList "file:computers.csv" -ModuleParameters $Parameters -SelectInvType reg
