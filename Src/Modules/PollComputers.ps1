...
function PollComputers {
    ...
    foreach($computer in $ComputerList.ComputerName)
    {
        ...
        if ($test_ws)
        {
            ...
            $inv_res | ForEach-Object {
                                        #имя сохраняемого файла
                                        $FileToSave = Join-Path $SaveInvPath "$($Computer).$($_.Type).csv"
                                        
                                        #если данные есть сохраняем их в файл
                                        if ($_.Data)
                                        {
                                            $_.Data | Export-csv -Path $FileToSave -Encoding UTF8 -NoTypeInformation
                                        }
                                        #если данных нет - удаляем старый файл с данным типов инвентаризации (т.е. если был выполнен сбор, но данных нет - значит и предыдущих данных быть не должно) 
                                        else
                                        {
                                            Remove-Item $FileToSave -Force -ErrorAction Ignore
                                        }
                                      }
