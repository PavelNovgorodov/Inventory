...
<#
    ...
    Настройка модуля
        ...
        Например, 
            ("s37fs03", 3), ("s37fs04", 4) - определить уровень вложенности для указанных компьютеров
            или
            ("s37fs*", 3) - определить уровень вложенности для компьютеров с именами по маске
...
if ($DepthAnyComputer)
{
    ...
    if ($DepthThisComputer) {
        ...
        Write-Host "Для $ComputerName получено значение параметра DepthChildFolder = $DepthChildFolder модуля shr." 
    }
}
...
foreach($share in $shares)
{
    ...
    if ($Psver -gt 4)
    {
        if ($LocalComputer)
        {
            $security = Get-SmbShareAccess -Name $share.name | select AccountName, AccessControlType, AccessRight
        }
        else
        {
    ...
    #если версия PowerShell 4 или ниже используем Get-WmiObject
    {
        if ($LocalComputer)
        {
            $ShareSecurity = Get-WmiObject -ClassName "Win32_LogicalShareSecuritySetting" -Filter "Name='$($share.name)'"
        }
        else
        {
    ...
    #если у шары есть параметр path и параметр $DepthChildFolder больше -1, определяем разрешения NTFS для каталогов
...
if (!$r) {$r = $null}
$r = [pscustomobject]@{Type = $CurrnetTypeInfo; Data = $r}
return $r