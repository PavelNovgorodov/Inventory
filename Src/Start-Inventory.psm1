...
.PARAMETER ModuleParameters
    ...
    Параметр передается в виде массива хэштаблиц:
                $Parameters = @(@{Module   = "Тип модуля"; VarName  = "Наименование переменной"; VarValue = "Значение переменной"})
    Например,         
                $Parameters = @(
                                 @{Module   = "shr"; VarName  = "DepthChildFolder"; VarValue = 2}, #передать в модуль shr меременную DepthChildFolder значение 2
                                 @{
                                    Module = "reg";  #передать в модуль reg
                                    VarName = "Reg"; #переменную Reg
                                    VarValue = @(    #со значениями
